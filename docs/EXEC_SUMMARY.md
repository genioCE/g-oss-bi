[EXEC_SUMMARY_bi.md](https://github.com/user-attachments/files/22052476/EXEC_SUMMARY_bi.md)
...
- **Module:** g-oss-bi (BI & dashboards)
- **Purpose:** Apache Superset connected to Trino for BI without per-viewer licenses.
- **Primary components:** Apache Superset (Trino SQLAlchemy connector)
- **Replaces / reduces:** Some Tableau/Power BI use cases (begin with net-new dashboards)
- **Integrates with:** g-oss-core (Trino), g-oss-batch (dbt models), g-oss-security (SSO)
...
```
[Gold models in Iceberg] ← Trino → Superset (dashboards/RLS)
```

This repository delivers the **g-oss-bi (BI & dashboards)** segment of that picture.

---

## 4) Common oil & gas use cases (examples)

- Production KPIs and allocation views
- Finance dashboards over curated marts
...
- Track **dashboard load time p95** and **query cache hit rate**; adopt RLS as needed.
...
1. **Pilot (0–90 days)**  
   - Stand up `g-oss-bi (BI & dashboards)` in a sandbox; connect a *low-risk* data domain.  
...
- **User adoption** → Train analysts; keep Tableau/Power BI where mandated.
- **RLS complexity** → Prefer schema/catalog-level controls in Trino.
- **Sprawl** → Govern via review process and folder permissions.
...
- Tableau/Power BI (subset) → Superset
